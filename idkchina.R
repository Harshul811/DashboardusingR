...
# Filter data for China to India trade
china_to_india_exports <- china_exports %>%
  filter(importer_name == "India")

china_to_india_imports <- china_imports %>%
  filter(exporter_name == "India")

# Top 10 Export Commodities from China to India by value
top_export_india <- china_to_india_exports %>%
  group_by(product_name) %>%
  summarise(total_export_value = sum(value, na.rm = TRUE)) %>%
  arrange(desc(total_export_value)) %>%
  slice_max(total_export_value, n = 10)

# Top 10 Import Commodities from China to India by value
top_import_india <- china_to_india_imports %>%
  group_by(product_name) %>%
...
top_export_commodities <- china_exports %>%
  group_by(product_name) %>%
  summarise(total_export_value = sum(value, na.rm = TRUE)) %>%
  arrange(desc(total_export_value)) %>%
  slice_max(total_export_value, n = 10)
...
top_import_commodities <- china_imports %>%
  group_by(product_name) %>%
  summarise(total_import_value = sum(value, na.rm = TRUE)) %>%
  arrange(desc(total_import_value)) %>%
  slice_max(total_import_value, n = 10)
...
ui <- dashboardPage(
  dashboardHeader(title = "Trade with China"),
  dashboardSidebar(
    sidebarMenu(
    ...
    tabItems(
      ...
      tabItem(tabName = "dashboard",
              fluidRow(
                ...
                column(6, 
                       div(class = "kpi-card", 
                           highchartOutput('TopExportIndia', height = "300px"))),
                column(6, 
                       div(class = "kpi-card", 
                           highchartOutput('TopImportIndia', height = "300px")))
              )
      ),
...
server <- function(input, output) {
  ...
  output$TopExportCommodities <- renderHighchart({
    hchart(top_export_commodities, "column", hcaes(x = product_name, y = total_export_value)) %>%
      hc_title(text = "Top 10 Export Commodities from China") %>%
  ...
  output$TopImportCommodities <- renderHighchart({
    hchart(top_import_commodities, "column", hcaes(x = product_name, y = total_import_value)) %>%
      hc_title(text = "Top 10 Import Commodities into China") %>%
  ...
  output$TopExportCommoditiesTimeSeries <- renderHighchart({
    export_time_series <- china_exports %>%
  ...
  output$TopImportCommoditiesTimeSeries <- renderHighchart({
    import_time_series <- china_imports %>%
  ...
  # World Trade Map for Exports from China
  output$WorldExportMap <- renderHighchart({
    world_export_data <- china_exports %>%
    ...
    hcmap(
      ...
      hc_title(text = "Global Export Trade Map with China")
  })
  
  # World Trade Map for Imports to China
  output$WorldImportMap <- renderHighchart({
    world_import_data <- china_imports %>%
    ...
    hcmap(
      ...
      hc_title(text = "Global Import Trade Map for China")
  })
  
  # Time Series Chart for Exports Over Time
  output$ExportsTimeSeries <- renderHighchart({
    export_time_series <- china_exports %>%
      group_by(year) %>%
      summarise(export_value = sum(value, na.rm = TRUE), .groups = "drop")
    
    hchart(export_time_series, "line", hcaes(x = year, y = export_value)) %>%
      hc_title(text = "Total Exports from China Over Time") %>%
      hc_xAxis(title = list(text = "Year")) %>%
      hc_yAxis(title = list(text = "Export Value (USD)")) %>%
      hc_tooltip(valuePrefix = "$")
  })
  
  # Time Series Chart for Imports Over Time
  output$ImportsTimeSeries <- renderHighchart({
    import_time_series <- china_imports %>%
      group_by(year) %>%
      summarise(import_value = sum(value, na.rm = TRUE), .groups = "drop")
    
    hchart(import_time_series, "line", hcaes(x = year, y = import_value)) %>%
      hc_title(text = "Total Imports into China Over Time") %>%
      hc_xAxis(title = list(text = "Year")) %>%
      hc_yAxis(title = list(text = "Import Value (USD)")) %>%
      hc_tooltip(valuePrefix = "$")
  })
  
  # Bar Chart for Top 10 Export Commodities from China to India
  output$TopExportIndia <- renderHighchart({
    hchart(top_export_india, "column", hcaes(x = product_name, y = total_export_value)) %>%
      hc_title(text = "Top 10 Export Commodities from China to India") %>%
      hc_xAxis(title = list(text = "Commodity"), categories = unique(top_export_india$product_name)) %>%
      hc_yAxis(title = list(text = "Export Value (USD)")) %>%
      hc_tooltip(pointFormat = "Export Value: {point.y} USD") %>%
      hc_legend(enabled = FALSE)
  })
  
  # Bar Chart for Top 10 Import Commodities from China to India
  output$TopImportIndia <- renderHighchart({
    hchart(top_import_india, "column", hcaes(x = product_name, y = total_import_value)) %>%
      hc_title(text = "Top 10 Import Commodities from China to India") %>%
      hc_xAxis(title = list(text = "Commodity"), categories = unique(top_import_india$product_name)) %>%
      hc_yAxis(title = list(text = "Import Value (USD)")) %>%
      hc_tooltip(pointFormat = "Import Value: {point.y} USD") %>%
      hc_legend(enabled = FALSE)
  })
